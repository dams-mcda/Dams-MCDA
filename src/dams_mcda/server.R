...
server <- function(input, output, session) {
	...
	generateOutput <- function (){
	    ...
	    if ( !damsCompleted(session$userData[['dams_completed']]) ){
			...
			# WSM expects criteria as column, dam as row. invert the matrix
			RawCriteriaMatrix <- t(RawCriteriaMatrix)

			# origial scores in table form, for debugging table size
			#output$FilledCriteriaTable <- renderTable(RawCriteriaMatrix, rownames=enable_rownames)

			# Call WSM (debug)
			message("Call WSM (debug)")
			results <- WSM(RawCriteriaMatrix, DamsData)

			# results 3 is 3d matrix
			message("WSM (debug) output damdata", results[3])
			output$DamsDataMatrixKeepMaintain <- renderTable(results[3][,,1], rownames=enable_rownames)
